...
- 作答畫面重新集中為單一遊戲舞台。
- 挑戰分類與題數改為可收合的精簡控制列；開始挑戰後自動收合。
- 記分列、候選字、同心圓與操作按鈕緊密排列，減少作答時上下捲動。
- 提交、清除與下一題固定置於同心圓正下方；答案地圖改列於操作區之後。
- 手機版記分資訊改為單列摘要，保留同心圓的主要畫面空間。
...
V0.9 僅調整前端版面與控制區收合行為。將新版網站檔案更新至 GitHub 即可，不需要執行新的 SQL。
